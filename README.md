...
- 支持阻塞判断：全局黑名单、唤醒冷却、QQ 机器人账号段过滤、复读 bot 最近发言过滤、wakepro 默认屏蔽词和自定义屏蔽词过滤。
- 支持指令屏蔽：可拦截内置指令、前缀命令消息、前缀普通消息，避免把命令或 `/随便聊聊` 这类消息误交给 LLM。
...
- `general`：总开关。
- `wake`：阻塞判断、wakepro 默认屏蔽词开关、自定义屏蔽词、指令屏蔽、消息防抖、提及唤醒、唤醒词触发方式、禁用纯引用唤醒、黑名单。

唤醒增强里几个容易混淆的开关：

- `use_default_block_keywords`：是否启用 wakepro 自带的默认屏蔽词。
- `block_keywords`：你自己额外补充的屏蔽词。
- `block_prefix_commands`：屏蔽 `/qq_avatar` 这类“前缀 + 指令”的消息。
- `block_prefix_llm`：屏蔽 `/帮我写个文案` 这类“前缀 + 普通聊天”的消息，但不影响真正的指令。
