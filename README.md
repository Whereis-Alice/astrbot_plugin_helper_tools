...
更新到 `v0.10.3` 后请重载插件。AstrBot 会根据 `requirements.txt` 安装模块所需依赖；手动部署时可在插件目录执行：
...
- **壁纸库**：单独管理每个图库的本地目录、随机抽图指令、文案、发送方式和子目录扫描；图片墙优先展示缩略图，可搜索排序、预览、下载、批量上传、重命名或删除图片。删除图库可选择只删配置，或在输入图库名称确认后连同磁盘目录一起删除。
