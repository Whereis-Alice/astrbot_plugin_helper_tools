...
AstrBot 会把引用消息中的图片也作为 LLM 的图片附件。开启本插件的 `reply_media_guard.enabled` 后，用户引用 bot 自己发出的图片时，插件会保留引用的文字内容，但不会把那张图片当成本次用户上传的图片传给模型。bot 自己直接发送的图片和其它插件自动发送的图片都能识别；用户正常随消息上传的图片不受影响。
...
- `general`：总开关。
- `reply_media_guard`：引用 bot 图片时，不把它误当作用户上传图片传给 LLM。
