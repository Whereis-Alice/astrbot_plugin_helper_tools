...
<img src="pages/helper-tools/logo-mark.svg" alt="辅助工具合集" width="84">

为 AstrBot 提供一组可由 LLM 主动调用、也可通过消息或命令使用的 QQ / OneBot 辅助能力。插件按模块组织配置，当前包含 X/Twitter 资料检索、网页浏览、环境感知、QQ 防撤回、群聊历史检索、QQ 信息、QQ 名片点赞、戳一戳互动、今日小猪、引用媒体标注、Anime1、收款码、随机语音、Steam、唤醒增强、本地壁纸和 Bot QQ 资料管理。
...
从 v2.0.0 开始，B 站视频理解、B 站专栏理解以及 QQ 引用小程序/音乐卡片识别已经移到独立插件 [astrbot_plugin_bilibili_card_parser](https://github.com/Whereis-Alice/astrbot_plugin_bilibili_card_parser)。这样可以单独启停媒体解析能力，避免与 Helper 的 QQ/唤醒功能互相影响。

安装新插件后，它会以复制、不删除的方式读取旧 Helper 的配置、B 站扫码凭据和 cookies.txt。旧插件的数据不会被清理；迁移结果只保存在新插件自己的数据目录中。安装新插件后请在其独立配置页调整 B 站和卡片模块，Helper 控制台不会再展示这些旧配置。
...
| 模块 | 主要能力 |
| --- | --- |
| 引用媒体识别 | 标注机器人自己发送的引用图片来源，降低自引用图片造成的误判；不保存图片正文到审计记录 |
...
| 环境感知 | 将当前时间、节假日、农历、节气、平台、可选的当前发言者群内资料和 Bot 自己的群身份作为本轮可信信息交给模型，默认关闭 |
...
| 引用媒体标注 | 标注机器人自己发送的引用图片来源，避免把自发图片误认为用户提供的资料 |
| Anime1 | 查询剧集更新和观看地址 |
...
更新到 `v2.0.0` 后请重载插件。AstrBot 会根据 `requirements.txt` 安装模块所需依赖；手动部署时可在插件目录执行：
...
- **概览**：显示已启用模块、当前实际可调用的 LLM 工具、本地数据占用、今日运行记录，以及 X/Twitter、群聊历史、壁纸图库和自动换头像任务等状态。
...
v1.0.0 起控制台改成「顶栏 + 横向标签栏」的单屏布局，替换了此前的左侧竖向侧栏：

- 整页固定为一个视口高度（`100dvh`），只有中间的内容区滚动，页面本身不会出现第二条滚动条。顶部品牌区与操作按钮、标签栏、底部状态栏始终可见，不必为了点“保存配置”先滚回顶部。
- 六个标签（概览、模块配置、运行记录、壁纸库、存储诊断、说明）横向排列，激活项用下划线标记；模块配置、运行记录、壁纸库三个标签会显示数字角标，分别对应已启用模块数、今日运行记录数和已索引图片数，为 0 时不显示。
- 底部状态栏同时显示当前操作反馈、常用标签的快捷入口和插件版本号。
- 配色改为深青绿主色的深色主题，并保留成套的浅色主题；两套主题都可在顶栏右侧切换，选择会保存到插件配置。
- 窄屏（手机）下标签栏可横向滑动，顶栏按钮收成图标，壁纸库的上传与筛选工具仍默认收起。

插件自带图标位于 [`pages/helper-tools/logo.svg`](pages/helper-tools/logo.svg)（158×48 的横向锁定版，图形加中文「辅助工具」与拉丁 HELPER / TOOLS；另有 48×48 的紧凑版 [`logo-mark.svg`](pages/helper-tools/logo-mark.svg)）。图形是一把 45° 斜置的两用扳手，右上是梅花头（闭口环加六角套筒），左下是开口头（U 形卡口），中间柄沿 45° 轴一笔连成，没有容器背板；两用扳手对应「一件工具应付多种规格」，也就是这个插件本身是一整套辅助工具的合集。图标采用单色渐变，深色与浅色两套配色都取自控制台主题，默认跟随系统外观自动切换，控制台顶栏则把图形内联进 DOM 后随顶栏主题切换，16px 下仍可辨识。本文顶部展示的是紧凑版：GitHub 无法把 SVG 内部的 `prefers-color-scheme` 对齐到自己的明暗主题，只有纯图形的紧凑版在两种主题下都读得出来，带文字的横版留给能控制底色的场合。

AstrBot 插件管理页的卡片图标另走一条路：AstrBot 只读取插件根目录下的 `logo.png`，文件名是硬编码的，既不能改名也不吃 SVG。本仓库的 [`logo.png`](logo.png) 由 [`logo.svg`](logo.svg)（512×512 方形瓦片）导出，图形与控制台图标同几何；因为 PNG 没法跟随控制台明暗主题，瓦片固定用深底加亮色标识，深浅两种主题下都成立。要换图标请改 `logo.svg` 再重新导出 PNG，不要直接编辑位图。
...
| `include_sender_group_profile` | 可选提供当前发言者在当前群的 QQ昵称、群昵称、身份、等级、专属头衔等资料 |
| `sender_group_profile_cache_seconds` | 当前发言者群内资料缓存秒数，默认 `60`；设为 `0` 则每轮重新查询 |
...
按群设置的目标会优先于默认目标。防撤回模块不读取或修改聊天历史模块的数据；消息与图片快照只保留到配置的时限，重载后也会自动清理过期文件。原消息如果带 QQ 引用段，重发时会转换为静态的“【引用消息】”文本，不再携带原消息 ID，避免 QQ 显示“原消息已过期”；正文、图片和表情仍会在同一条消息中发送。
...
- 对于其它插件自动发送后、引用段里的发送者或图片链不可靠的情况，插件会按引用消息 ID 调用 OneBot `get_msg` 核对真实发送者和图片段，并兼容常见的 `data`、`result` 返回包装。来源说明会在消息阶段尽早写入，并在真正发送 LLM 请求前作为本轮系统提示注入，避免把旧图误当成当前用户上传；图片本身仍会正常传给模型识图。
...
- 被引用的小程序、音乐和普通分享卡片的结构化读取已移至独立的 `astrbot_plugin_bilibili_card_parser` 插件；本模块只负责引用图片来源标注。
...
管理员唤醒词 `wake.admin_wake_words` 默认为空。v1.0.0 之前它内置了 `宝贝`、`宝宝` 两个词，且清空后会静默回落到这份默认值，导致管理员随口说这两个词就会唤醒模型且关不掉；现在留空就是真的不启用管理员专属唤醒词。同批修正的还有 `voice.trigger_keywords`、`qq_like.trigger_phrases`、`qq_like.mention_trigger_prefixes`、`anime1.update_times`，这些列表现在都能被清空并保持为空。
...
- 图片删除和“只删除图库配置”等危险操作使用控制台内置确认框，不依赖可能被 AstrBot 内嵌页面拦截的浏览器原生弹窗；删除图片会同步删除对应磁盘文件。
- 新增或编辑图库时可直接设置名称、目录、随机抽图指令、文案、发送方式和“扫描子目录”。目录不存在时可勾选自动创建；删除图库可选择“仅删配置，保留磁盘文件”，或“删除配置和图库目录中的图片文件”。后者必须输入完全一致的图库名称确认，只会删除被索引到的图片（受该图库的扩展名白名单和“扫描子目录”设置约束），目录内的其它文件一律保留，只有因删图而变空的目录会被回收；同时会拒绝用户主目录、插件数据根目录、`wallpapers` 总目录、文件系统根、符号链接和与其它图库重叠的路径。
- 图库管理器保存后会立即同步到模块配置；即使随后点击页面底部的“保存配置”，也不会用旧页面状态覆盖刚新增的图库。
...
- `aiohttp`：X/Twitter、历史回填等异步网络请求。
- `Pillow`：图片校验、今日小猪和壁纸处理。
