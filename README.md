...
v0.12.0 起控制台改成「顶栏 + 横向标签栏」的单屏布局，替换了此前的左侧竖向侧栏：
...
插件自带图标位于 [`pages/helper-tools/logo.svg`](pages/helper-tools/logo.svg)（另有 48×48 的紧凑版 [`logo-mark.svg`](pages/helper-tools/logo-mark.svg)）。图形是一块圆角控制面板加一个几何化的「工」字，取“工具”与“工字梁——承重的辅助结构”两层意思，深色和浅色底各有一套配色，控制台顶栏直接内联使用。
...
管理员唤醒词 `wake.admin_wake_words` 默认为空。v0.12.0 之前它内置了 `宝贝`、`宝宝` 两个词，且清空后会静默回落到这份默认值，导致管理员随口说这两个词就会唤醒模型且关不掉；现在留空就是真的不启用管理员专属唤醒词。同批修正的还有 `voice.trigger_keywords`、`qq_like.trigger_phrases`、`qq_like.mention_trigger_prefixes`、`anime1.update_times`，这些列表现在都能被清空并保持为空。
